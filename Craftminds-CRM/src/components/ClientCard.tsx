import { Draggable } from '@hello-pangea/dnd';
import type { Client } from '../types';
import { PencilIcon, EnvelopeIcon, BuildingOfficeIcon } from '@heroicons/react/24/outline';
import { useState } from 'react';
import { ClientFormModal } from './AddClientForm';

interface ClientCardProps {
  client: Client;
  index: number;
  onEdit?: (updated: Client) => void;
}

const ClientCard = ({ client, index, onEdit }: ClientCardProps) => {
  const [editOpen, setEditOpen] = useState(false);
  return (
    <Draggable draggableId={client.id} index={index}>
      {(provided) => (
        <div
          ref={provided.innerRef}
          {...provided.draggableProps}
          {...provided.dragHandleProps}
          className="card-glass group relative"
        >
          <button
            className="absolute top-2 right-2 p-1.5 text-gray-400 hover:text-blue-400 hover:bg-blue-900/30 rounded-full transition-colors duration-200 opacity-0 group-hover:opacity-100"
            onClick={() => setEditOpen(true)}
            title="Éditer"
            type="button"
          >
            <PencilIcon className="h-4 w-4" />
          </button>
          <div className="space-y-1">
            <h3 className="font-semibold text-gray-100 text-base">{client.name}</h3>
            <div className="flex items-center text-xs text-gray-400 space-x-2">
              <EnvelopeIcon className="h-4 w-4" />
              <span>{client.email}</span>
            </div>
            <div className="flex items-center text-xs text-gray-400 space-x-2">
              <BuildingOfficeIcon className="h-4 w-4" />
              <span>{client.company}</span>
            </div>
          </div>
          {client.notes && (
            <div className="mt-3 pt-3 border-t border-gray-800">
              <p className="text-xs italic text-gray-300 line-clamp-2">{client.notes}</p>
            </div>
          )}
          {editOpen && (
            <ClientFormModal
              initialValues={client}
              onClose={() => setEditOpen(false)}
              onSubmit={updated => onEdit && onEdit({ ...client, ...updated, updatedAt: new Date().toISOString() })}
              title="Éditer le client"
              submitLabel="Enregistrer"
            />
          )}
        </div>
      )}
    </Draggable>
  );
};

export default ClientCard; 